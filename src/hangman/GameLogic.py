    ...
    async def check_for_gamestart(self):
        if self.queue.__len__() > 1:
            ...
            if self.queue.__len__() == 2:
                self.timer = Timer(60, self.start_game(self.queue.queue, channelid))
                # TODO: SEND MESSAGE TO PLAYERS THAT TIMER HAS STARTED
            if not channelid == False:
                if self.queue.__len__() == 8:
                    self.start_game(self.queue.queue,channelid)
                    self.queue.queue.clear()
        else:
            if self.timer is not None:
                self.timer.cancel()
                # TODO: SEND MESSAGE TO PLAYERS THAT THE TIMER HAS CANCELED BECAUSE TO FEW PLAYERS

    def start_game(self, playerids, channelid):
        hangmangame = Game(playerids, channelid, self.bot)
        self.bot.add_cog(hangmangame)
        self.games.append(hangmangame)
        # TODO: SEND MESSAGES TO PLAYERS THAT GAME HAS STARTET IN CHANNEL CHANNEL
        return
